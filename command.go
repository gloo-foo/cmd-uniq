...
func Uniq(opts ...any) gloo.Command[[]byte, []byte] {
	...
	return patterns.Accumulate(func(lines [][]byte) ([][]byte, error) {
		return collapse(lines, bool(f.ignoreCase), render), nil
	})
}

// collapse folds adjacent equal lines into groups, then emits each selected
// group through render.
func collapse(lines [][]byte, ignoreCase bool, render func(group) ([]byte, bool)) [][]byte {
	out := make([][]byte, 0, len(lines))
	for _, g := range groupLines(lines, ignoreCase) {
...
func groupLines(lines [][]byte, ignoreCase bool) []group {
	groups := make([]group, 0, len(lines))
	for _, line := range lines {
		if n := len(groups); n > 0 && sameLine(groups[n-1].rep, line, ignoreCase) {
			groups[n-1].count++
			continue
		}
...
func sameLine(a, b []byte, ignoreCase bool) bool {
	if ignoreCase {
...
func selector(f flags) func(group) bool {
	keep := keepAll
	if bool(f.duplicatesOnly) {
		keep = and(keep, isRepeated)
	}
	if bool(f.uniqueOnly) {
...
func formatter(withCount bool) func(group) []byte {
	if withCount {
		return countLine
	}
	return plainLine
