...
func lines(t *testing.T, input run.Input, opts ...any) []string {
	t.Helper()
...
func TestUniq_Count_MultiDigitWidth(t *testing.T) {
	...
	got := lines(t, run.Input(strings.Repeat("z\n", 10)), command.UniqCount)
...
// ==============================================================================
// Non-option arguments pass through to the framework classifier
// ==============================================================================

func TestUniq_NonOptionArgumentIsClassifiedNotFolded(t *testing.T) {
	// A positional argument (a File-convertible string) is not one of uniq's
	// options: it must pass through the option fold untouched, leaving every
	// flag at its default, and the command still reads the piped stream.
	got := lines(t, "a\na\nb\n", "ignored.txt")
	assertion.Lines(t, got, []string{"a", "b"})
}
