...
Highly compatible with GNU `uniq` for the implemented flags. Output is byte-identical to GNU `uniq` for the default collapse, `-c`, `-d`, `-u`, and `-i` (verified in the `yupsh/bin-uniq` Docker integration harness against Debian coreutils).
